...
import { uploadMedia, downloadMedia, getManifestEntry } from './spine.js';
...
window.addEventListener('beforeunload', () => {
  for (const { objectURL } of _mediaCache.values()) {
...
// Download and decrypt a media file. Returns an object URL.
// Result is cached in memory — subsequent calls for the same UUID return
// the cached URL instantly without hitting Drive again.
export async function fetchMedia(uid, encKey, uuid) {
  ...
  evictMediaCache();
  _mediaCache.set(uuid, { objectURL, mimeType: blob.type });
  return objectURL;
}

// Manually evict a specific UUID from the cache (e.g. after file deletion)
export function evictMediaFromCache(uuid) {
  const entry = _mediaCache.get(uuid);
  if (entry) { URL.revokeObjectURL(entry.objectURL); _mediaCache.delete(uuid); }
}
