...
const CACHE_MAX = 50, CACHE_TARGET = 40;
function evictMediaCache() {
  if (_mediaCache.size <= CACHE_MAX) return;
  for (const key of _mediaCache.keys()) {
    _mediaCache.delete(key);
    if (_mediaCache.size <= CACHE_TARGET) break;
  }
}
...
export function buildFileBubbleHTML(msg) {
  const safeUuid = (msg.uuid ?? '').replace(/'/g, '');  // UUIDs are hex+dash; strip any unexpected chars
  return `
    <div class="media-bubble media-file" onclick="Nexus.openMedia('${safeUuid}')">
