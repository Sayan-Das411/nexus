...
export function applyOtherBubbleColor(hex) {
  ...
  // Coordinate avatar-initial backgrounds with the received bubble colour.
  // A brightness boost ensures the initial circle is visually distinct from the
  // bubble background while still matching its hue.
  const boosted = adjustBrightness(hex, 0.35);
  document.documentElement.style.setProperty('--avatar-initial-bg', boosted);
}
...
export function applyVideoWallpaper(dataUrl) {
  ...
  const handleVis = () => {
    ...
    else if (!video._userPaused) video.play().catch(() => {});
  };
...
// Pause / resume the video wallpaper without tearing down the element.
// Used when overlays with backdrop-filter are shown — keeping the video
// playing under a blur filter causes severe frame-rate drops on most devices.
export function pauseVideoWallpaper() {
  if (_videoWallEl && !_videoWallEl.paused) {
    _videoWallEl.pause();
    _videoWallEl._userPaused = true;
  }
}

export function resumeVideoWallpaper() {
  if (_videoWallEl && _videoWallEl._userPaused) {
    _videoWallEl._userPaused = false;
    if (!document.hidden) _videoWallEl.play().catch(() => {});
  }
}
...
let _slideshowIntervalMs = 30000;  // stored for pause/resume
...
export function startSlideshowWallpaper(dataUrls, intervalMs = 30000, opts = {}) {
  ...
  _slideshowIntervalMs = intervalMs;
...
// Pause / resume the slideshow timer without tearing down layers.
// Used alongside pauseVideoWallpaper when overlays are open.
let _slideshowPaused = false;

export function pauseSlideshowWallpaper() {
  if (_slideshowTimer) {
    clearInterval(_slideshowTimer);
    _slideshowTimer = null;
...
export function resumeSlideshowWallpaper() {
  if (_slideshowPaused && !_slideshowTimer && _slideshowImages.length > 1) {
    _slideshowTimer = setInterval(_advanceSlide, _slideshowIntervalMs);
...
export async function applyStoredAppearance(uid) {
  ...
  if (otherBubble) {
    applyOtherBubbleColor(otherBubble);
  } else {
    // No custom received-bubble colour — clear any stale avatar-initial override
    // so initials fall back to the accent gradient.
