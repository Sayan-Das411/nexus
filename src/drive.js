...
export function clearDriveToken(driveUid) {
  try { localStorage.removeItem(TOKEN_PREFIX + driveUid); } catch (e) { /* ignore */ }
}
...
export async function uploadFile(driveUid, folderId, uuid, encryptedBlob, onProgress) {
  ...
  if (encryptedBlob.size < 5 * 1024 * 1024) {
    return uploadMultipart(token, folderId, uuid, encryptedBlob, onProgress);
  } else {
...
function xhrUpload(method, url, headers, body, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      _activeXhr = null;
      if (xhr.status === 401) { reject(new DriveAuthError()); return; }
      if (xhr.status === 403) {
        let msg = '';
        try { msg = JSON.parse(xhr.responseText).error.message || ''; } catch (e) { /* ignore */ }
...
async function uploadMultipart(token, folderId, uuid, blob, onProgress) {
