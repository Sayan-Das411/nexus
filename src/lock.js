...
export async function getLockSettings(uid) {
  return (await lockStore.get(uid)) ?? {};
}
...
let _visibilityLockSetUp = false;
export function setupVisibilityLock() {
  if (_visibilityLockSetUp) return; // only wire once, even across account switches
  _visibilityLockSetUp = true;
  ...
  document.addEventListener('visibilitychange', async () => {
    if (document.hidden) {
      ...
      else resetAutoLockTimer(); // resume timer for remaining time
    }
  });
