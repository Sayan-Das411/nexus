  ...
  try {
    ...
    if (S.isOnline) {
      try {
        ...
        // Firebase send failed — queue it and show pending bubble
        const qId = await queue.add(S.uid, payload);
        renderPendingBubble(payload, text, qId);
...
function renderPendingBubble(payload, plaintextForDisplay, queueId) {
