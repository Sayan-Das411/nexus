...
// Dedup set for markRead — cleared on account switch via clearMarkedRead()
const _markedRead = new Set();
export function clearMarkedRead() { _markedRead.clear(); }

function db() {
  // Re-derive on every call — getDatabase() is idempotent and O(1).
  // Caching in a module-level _db risks using a stale reference if
  // getFirebaseApp() returns a re-initialized app during onboarding.
...
export async function setupProfile(uid, deviceId, deviceName, platform,
                                   encryptionSalt, verificationVector, browser) {
...
export async function registerDevice(uid, deviceId, deviceName, platform, browser) {
  // Use update() instead of set() to preserve existing fields (e.g. avatar)
  // that may have been written by other flows. The device record is first
  // created by setupProfile() during onboarding; subsequent logins only need
  // to refresh the mutable fields.
...
// Pass extras (e.g. { browser }) to fix stale fields in the device record.
export async function touchDevice(uid, deviceId, extras = {}) {
  await update(ref(db(), P.device(uid, deviceId)), {
    lastSeen: serverTimestamp(),
    ...extras,
  });
...
// Store or clear an encrypted avatar on the device record.
// encryptedAvatarJson = JSON string { ciphertext, iv } or null to remove.
export async function updateDeviceAvatar(uid, deviceId, encryptedAvatarJson) {
  await update(ref(db(), P.device(uid, deviceId)), {
    avatar: encryptedAvatarJson ?? null,
  });
}

// Read just the avatar field for one device (used during initial avatar load).
export async function getDeviceAvatarField(uid, deviceId) {
  const snap = await get(ref(db(), `${P.device(uid, deviceId)}/avatar`));
  return snap.exists() ? snap.val() : null;
}
...
export async function sendMessage(uid, payload) {
  ...
  if (payload.fileSize != null) record.fileSize = payload.fileSize;
...
//
// ⚠️  REQUIRED Firebase Security Rules — add .indexOn to silence the
//     "Using an unspecified index" warning and prevent full-table scans:
//
//   {
//     "rules": {
//       "accounts": {
...
//   }
//
//   Without this, Firebase downloads every message to the client before
//   sorting — performance degrades significantly as history grows.
...
// Deduped in memory to avoid redundant Firebase writes on every snapshot fire.
export async function markRead(uid, msgId, deviceId) {
  const key = `${uid}:${msgId}:${deviceId}`;
  if (_markedRead.has(key)) return;
  _markedRead.add(key);
  await set(ref(db(), P.readBy(uid, msgId, deviceId)), Date.now());
}

// Remove a device record and its presence entry.
// Called by the master-device management UI to sign out a linked device.
export async function removeDevice(uid, deviceId) {
  await remove(ref(db(), P.device(uid, deviceId)));
  await remove(ref(db(), P.presenceDevice(uid, deviceId)));
}
...
// Cancel any pending typing-clear timer. Call on account switch to prevent
// the timer from firing with a stale uid/deviceId.
...
export async function setTyping(uid, deviceId, isTyping) {
  ...
  // Capture uid+deviceId at schedule time — if the account changes before
  // the timer fires, clearTypingTimer() will have already cancelled this.
  if (isTyping) {
    const _uid = uid, _did = deviceId;
    _typingTimer = setTimeout(() => setTyping(_uid, _did, false), 6000);
  }
}
...
// @deprecated — Legacy profile-level avatar. The app now uses per-device avatars
// via updateDeviceAvatar(). Kept for backward compatibility with older clients.
...
// ---- Phase 4: Message mutations ---------------------------------------------

// Soft-delete a message. All devices replace its bubble with "This message was deleted."
// The encrypted ciphertext is removed; only the tombstone flags remain.
export async function deleteMessage(uid, msgId) {
  await update(ref(db(), P.message(uid, msgId)), {
    deleted:    true,
    ciphertext: null,
    iv:         null,
  });
...
// Edit a sent message. Re-encrypts on-device; all devices re-render the bubble.
export async function editMessage(uid, msgId, newCiphertext, newIv) {
  await update(ref(db(), P.message(uid, msgId)), {
...
// reactions/{emoji}/{deviceId} = timestamp | null
export async function reactToMessage(uid, msgId, deviceId, emoji) {
  const reactRef = ref(db(), `${P.message(uid, msgId)}/reactions/${emoji}/${deviceId}`);
...
// Stored in Firebase so they survive browser data clears and sync across devices.
// Values are plain strings/numbers — no sensitive data.

export async function savePreferences(uid, prefs) {
  await update(ref(db(), `accounts/${uid}/preferences`), prefs);
}

export async function loadPreferences(uid) {
  const snap = await get(ref(db(), `accounts/${uid}/preferences`));
  return snap.exists() ? snap.val() : null;
}
